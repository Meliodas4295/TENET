...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
        ...
        public GameObject CMCam2;
        public float distance;
        public float distanceGun;
		...
		Vector3 gunInitialPos;
		Quaternion gunInitialRot;

		public Animator bullet;
        public Animator fadeGun;
		public Animator g;
		...
		private void Start()
		{
			...
			fadeGun.enabled = false;
			gunInitialPos = gun.transform.position;
			gunInitialRot = gun.transform.rotation;
			speed = 0.05f;
			speedFade = 1f;
		...
		private void Update()
		{
			distanceGun = Vector3.Distance(transform.position, gun.transform.position);
            ...
            JumpAndGravity();
			GroundedCheck();
			if(distanceGun < 2 && Input.GetKeyDown(KeyCode.Return))
			{
                g.enabled = true;
                bullet.enabled = true;
                fadeGun.enabled = true;
                finalTimer = 0;
				cameraCM.SetActive(true);
				CMCam1.SetActive(true);
				CMCam2.SetActive(true);
				capsule.SetActive(false);
				gun.transform.position = posGunCM.transform.position;
            ...
            if (finalTimer > 7)
			{
                ...
                gun.transform.rotation =gunInitialRot;
                cameraCM.SetActive(false);
                CMCam1.SetActive(false);
                CMCam2.SetActive(false);
			...
			if(isBlock && rotatingRockController.isRockRotatingFinish)
			{
				...
				if (offsetTimer > 1.8f)
				{
                    transform.position = Vector3.Lerp(transform.position, inverseInteractionSpace.position, timer * speed);
			...
			if(timer > 1/0.1f)
			{
				...
				_playerInput.SwitchCurrentActionMap("Inverse");
				//isInverseWorld = true;
			}
